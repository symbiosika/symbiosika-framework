...
// simple KV store for active user tokens as a cache
const KNOWN_AND_VALID_TOKENS = new Map<
  string,
  { usersEmail: string; usersId: string }
>();
...
export async function verifyHankoToken(c: Context) {
  ...
  if (!KNOWN_AND_VALID_TOKENS.has(token)) {
    ...
    KNOWN_AND_VALID_TOKENS.set(token, {
      usersEmail: user.email,
      usersId: user.id,
    });
  }

  return KNOWN_AND_VALID_TOKENS.get(token)!;
}
