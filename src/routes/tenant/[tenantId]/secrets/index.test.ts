...
describe("Secrets API Endpoints", () => {
  ...
  test("should get all secrets", async () => {
    const response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/secrets`,
      userOrg1Token
    );
  ...
  test("should delete a secret", async () => {
    ...
    const deleteResponse = await testFetcher.delete(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/secrets/SECRET_TO_DELETE`,
      userOrg1Token
    );
  ...
  test("should handle invalid requests", async () => {
    ...
    const nonExistentResponse = await testFetcher.delete(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/secrets/NON_EXISTENT_SECRET`,
      userOrg1Token
    );
...
describe("Security checks for Secrets API Endpoints", () => {
  // Test unauthorized access to another tenant's secrets
  test("should not allow user from another tenant to get secrets", async () => {
    const response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/secrets`,
      userOrg2Token
  ...
  test("should not allow user from another tenant to set a secret", async () => {
    const secretData = {
  ...
  test("should not allow user from another tenant to delete a secret", async () => {
