...
export default function defineManageSecretsRoutes(
  ...
  app.get(
    API_BASE_PATH + "/tenant/:tenantId/secrets",
    authAndSetUsersInfo,
    checkUserPermission,
    describeRoute({
      tags: ["secrets"],
      summary: "Get all secrets for an tenant",
      responses: {
        200: {
    ...
    async (c) => {
      try {
        ...
        const value = await getSecrets(tenantId);
        return c.json(value);
      } catch (error) {
  ...
  app.post(
    ...
    async (c) => {
      try {
        ...
        const secret = await setSecret({ ...parsed, tenantId });
        return c.json({
          id: secret.id,
          name: secret.name,
   ...
   */
  app.delete(
    API_BASE_PATH + "/tenant/:tenantId/secrets/:name",
    authAndSetUsersInfo,
    checkUserPermission,
    describeRoute({
      tags: ["secrets"],
    ...
    async (c) => {
      ...
      try {
        await deleteSecret(name, tenantId);
