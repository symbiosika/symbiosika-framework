...
describe("Webhook API Endpoints", () => {
  test("CRUD operations", async () => {
    ...
    console.log("should get all tenant webhooks");
    response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/global`,
      TEST_USER_1_TOKEN
    );
    expect(response.status).toBe(200);
    data = response.jsonResponse;
    ...
    response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/${createdWebhook.id}`,
      TEST_USER_1_TOKEN
    );
    console.log(response.textResponse);
    expect(response.status).toBe(200);
    ...
    response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/00000000-0000-0000-0000-000000000000`,
      TEST_USER_1_TOKEN
    );
    console.log(response.textResponse);
  ...
  test("n8n simulation", async () => {
    ...
    let response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/register/n8n`,
      TEST_USER_1_TOKEN,
      {
    ...
    response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/check`,
      TEST_USER_1_TOKEN,
      {
        webhookId: wh.id,
      }
    ...
    response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/webhooks/${wh.id}/trigger`,
      TEST_USER_1_TOKEN,
      {
        test: "payload",
      }
