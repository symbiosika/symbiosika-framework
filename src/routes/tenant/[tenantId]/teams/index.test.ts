...
describe("Teams API Endpoints", () => {
  test("Full CRUD and Permission Tests", async () => {
    ...
    const addedTeam = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/teams`,
      adminToken,
      {
    ...
    // ------------------------------------------------------------
    console.log("Step 2: Get all teams of an tenant");
    ...
    response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/teams/${addedTeamId}`,
      adminToken
    );
    // console.log(response.textResponse);
    expect(response.status).toBe(200);
    ...
    response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/teams/${addedTeamId}/members`,
      adminToken
    );
    // console.log(response.textResponse);
    expect(response.status).toBe(200);
    ...
    // ------------------------------------------------------------
    // add a member to a team. should fail because the user is not a member of the tenant
    // ------------------------------------------------------------
    console.log("Step 7: Add a member to a team");
    console.log(
      "Try to add a member to a team which is not a member of the tenant"
    );
    ...
    // ------------------------------------------------------------
    console.log(
      "Try to add a member to a team which is now a member of the tenant"
    );
    response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/teams/${addedTeamId}/members`,
