...
describe("Invitations Security Tests", () => {
  ...
  test("should reject unauthorized requests", async () => {
    rejectUnauthorized(app, [
      ["GET", `/api/tenant/${TEST_ORGANISATION_1.id}/invitations`],
      ["POST", `/api/tenant/${TEST_ORGANISATION_1.id}/invitations`],
      [
        "DELETE",
        `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${createdInvitationId}`,
      ],
      [
        "POST",
        `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${createdInvitationId}/accept`,
      ],
      [
        "POST",
        `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${createdInvitationId}/decline`,
      ],
    ]);
  });

  test("should reject access to invitations from other tenants", async () => {
    // User2 tries to access invitations from Organisation1
    const response = await testFetcher.get(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations`,
      user2Token
    );
  ...
  test("should reject invitation creation for other tenants", async () => {
  ...
  test("should reject invitation deletion from other tenants", async () => {
