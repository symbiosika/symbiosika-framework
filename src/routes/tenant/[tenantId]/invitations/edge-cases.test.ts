...
describe("Invitations Edge Cases and Error Handling", () => {
  ...
  beforeAll(async () => {
    ...
    await getDb()
      ...
      .where(
        and(
          eq(tenantInvitations.tenantId, TEST_ORGANISATION_1.id),
          eq(tenantInvitations.email, TEST_ORG3_USER_1.email)
        )
      );
  });
  ...
  test("should handle declining a non-existent invitation", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${nonExistentId}/decline`,
      u3Token,
      {}
    );
  ...
  test("should handle dropping a non-existent invitation", async () => {
    ...
    const response = await testFetcher.delete(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${nonExistentId}`,
      u1Token
    );
