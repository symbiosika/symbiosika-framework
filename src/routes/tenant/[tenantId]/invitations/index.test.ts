...
describe("Invitations API Endpoints", () => {
  ...
  test("should create a new invitation", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations?sendMail=false`,
      u1Token,
      invitationData
    );
    ...
    expect(response.jsonResponse.tenantId).toBe(TEST_ORGANISATION_1.id);
  ...
  // Test creating an invitation with mismatched tenantId
  test("should reject invitation with mismatched tenantId", async () => {
    const invitationData = {
      tenantId: TEST_ORGANISATION_2.id, // Different from URL param
      email: TEST_ORG3_USER_1.email,
      role: "member",
      status: "pending",
    };
  ...
  // Test getting all invitations for an tenant
  test("should get all invitations for an tenant", async () => {
  ...
  test("should accept an invitation", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${invitationId}/accept`,
      u2Token,
      {}
    );
  ...
  test("should accept all pending invitations for a user", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/all/accept`,
      u3Token,
      {}
    );
  ...
  test("should decline an invitation", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${invitationId}/decline`,
      u3Token,
      {}
    );
  ...
  test("should decline all pending invitations for a user", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/all/decline`,
      u3Token,
      {}
    );
  ...
  test("should drop an invitation", async () => {
    ...
    const response = await testFetcher.delete(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${invitationId}`,
      u1Token
    );
  ...
  test("should handle errors for non-existent invitation", async () => {
    ...
    const response = await testFetcher.post(
      app,
      `/api/tenant/${TEST_ORGANISATION_1.id}/invitations/${nonExistentId}/accept`,
      u3Token,
      {}
    );
